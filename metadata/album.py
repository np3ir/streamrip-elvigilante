        ...
        if isinstance(raw_artists, list) and raw_artists:
            ...
            for a in raw_artists:
                if not isinstance(a, dict):
                    logger.warning(
                        "AlbumMetadata.from_qobuz: unexpected artist entry type %s, skipping",
                        type(a),
                    )
                    continue
                name = a.get("name")
                if name and isinstance(name, str):
                    ...
                    logger.warning(
                        "AlbumMetadata.from_qobuz: artist entry missing/invalid 'name': %r, skipping",
                        a,
                    )
            albumartist = (
                artist_separator.join(names)
                if names
                else typed(safe_get(resp, "artist", "name"), str)
            )
        else:
