...
#### GitHub publication state and recommended next feature

- The artwork/LRC corrections were committed and pushed to `origin/main` as `7690dfd` (`fix: preserve lyrics and deduplicate artwork publication`). The canonical album metadata reuse and graceful TIDAL safety-stop corrections were subsequently committed and pushed to `origin/main` as `fca8208` (`fix: preserve canonical album metadata in library jobs`). Local `main` and `origin/main` matched after each push; GitHub Python, Ruff, and CodeQL workflows were triggered. No tag, release, PyPI publication, force push, or credential change occurred.
- The next recommended product feature is a persistent, incremental library index keyed primarily by normalized ISRC and recording physical quality. It should scan trusted roots explicitly, retain path/size/mtime/codec/bit-depth/sample-rate, validate indexed paths before skipping, report duplicates without modifying them, and update only new or changed files. Download planning should skip an indexed recording only when its existing physical quality satisfies the requested ceiling/policy. Metadata-only fallback must remain conservative when ISRC is absent.
- Proposed CLI surface: `rip library-index build ROOT`, `rip library-index update`, `rip library-index status`, and `rip library-index duplicates`. This would recognize files created by tiddl-elvigilante or other clients without repeatedly walking the network volume and would have prevented the observed duplicate ZZ Top box-set download when Streamrip's own database lacked those pre-existing ISRCs.

### Persistent ISRC index and resilient TIDAL session refresh

- The persistent incremental SQLite library index is implemented at the private application-data path and the explicitly requested full `Z:\` scan completed successfully. It registered 506,039 tagged audio files beneath canonical root `\\Servidor\server\Music`; the index contains path, normalized ISRC, size, mtime, codec, losslessness, bit depth, and sample rate. `build`, `update`, `status`, and read-only `duplicates` CLI operations are available. Download planning validates indexed paths and skips an ISRC only when the existing physical quality satisfies the selected candidate.
- A real one-track run for TIDAL track `498773237` successfully refreshed both Streamrip TIDAL sessions and selected FLAC 16-bit/44.1 kHz. The existing canonical audio was preserved, lyrics were unavailable, and the manifest completed with zero failures. Both private token files now record the matching OAuth `client_id`; no token value is recorded here.
- Local comparison with the concurrently functioning installed `tiddl-elvigilante` established that the earlier `403/12001 abuse_detected` applied to Streamrip's saved refresh session, not necessarily the whole account. Tiddl remains functional on the same account through separately persisted HiRes and TV sessions. Streamrip now labels this as a rejected OAuth session rather than declaring a global account suspension.
- Streamrip now follows the relevant tiddl refresh resilience: refresh uses HTTP Basic client authentication, omits the optional `scope`, records the originating client ID, writes rotated tokens through flush/fsync plus atomic replace, and retains a still-valid access token when only a preventive refresh receives the rejection. An expired token or forced refresh after 401 still fails closed. It does not silently copy tiddl's refresh tokens, avoiding rotation races between applications.
- The live run initially reached ordinary path-based `Skipped (Exists)` instead of indexed skipping even though the ISRC was present. Root cause was the remote candidate advertising stereo while the current index schema left channels unknown; full quality tuple ordering incorrectly treated unknown channels as worse. Indexed sufficiency now compares the requested measurable ceiling fields (lossless, bit depth, sample rate, or lossy bitrate) and does not force a duplicate download solely because an optional channel field is unknown.
- Validation after the TIDAL changes: uninterrupted suite `262 passed, 7 skipped`; Ruff passed. After the final indexed-quality correction, focused index/auth/CLI validation passed `28 passed`; Ruff and `git diff --check` passed apart from informational Windows line-ending notices. Current work remains uncommitted and unpushed; no media file was modified by the diagnostic checks.

#### Cancellation cleanup and stereo fallback policy

- An artist run for TIDAL artist `3915689` selected EAC3/Atmos as the highest-bitrate lossy fallback under a 16-bit ceiling and downloaded 12 `.m4a` files before the user interrupted it. The interruption returned to PowerShell but emitted four unclosed aiohttp-session errors. Existing downloaded files were not deleted or replaced during diagnosis.
- `Main.__aexit__` now runs worker and client shutdown in a separate shielded cleanup task. All client closers are gathered with per-client exception isolation, database handles and artwork temporary directories are finalized, and only then is cancellation re-raised. A regression test cancels cleanup while four clients are closing and proves every closer finishes exactly once.
- The agreed music-library hierarchy now excludes spatial lossy formats by default. New permanent comparison setting `allow_spatial = false` rejects Atmos/EAC3 unless explicitly enabled; `rip library` exposes `--allow-spatial` and `--no-spatial`. TIDAL's internal lossy cascade prefers ordinary stereo AAC over a higher-bitrate EAC3 response, while verified lossless still wins. Indexed EAC3 is not accepted as a substitute for requested stereo AAC.
- Focused cancellation/config/quality tests passed `76 passed`; the subsequent uninterrupted full suite passed `266 passed, 7 skipped`, and Ruff passed. One final focused indexed-EAC3 regression was added afterward. Changes remain uncommitted and unpushed; no further remote request or download was performed.

### Persistent ISRC library index and TIDAL session resilience

- The persistent incremental SQLite library index is now implemented at the private application-data location `library-index.db`. It records normalized ISRC, canonical path, size, modification time, codec, lossless state, physical bit depth, and sample rate. Scans parse only new or changed audio, remove stale entries, refresh a modified match on demand, report duplicates without modifying files, and skip a planned download only when the indexed physical quality equals or exceeds the selected candidate.
- CLI commands `rip library-index build ROOT`, `update`, `status`, and `duplicates` are registered. A complete explicit scan of the configured network library finished with 506,039 indexed audio files under `\\Servidor\server\Music`; a prior focused ZZ Top scan indexed 732 files and proved incremental behavior. No media file was modified by either scan.
- A TIDAL refresh response reported `403/12001 abuse_detected` for Streamrip while two `tiddl-elvigilante` processes using the same account continued downloading successfully. This disproves a global-account diagnosis: the verified scope is the saved Streamrip OAuth session/refresh attempt. The CLI now reports this as a rejected Streamrip session without a traceback and does not claim other sessions are unavailable.
- Review of the locally installed tiddl-elvigilante implementation identified its relevant resilience contract: retain a still-valid access token after a failed preventive refresh, stop repeated proactive refresh after a 4xx, omit `scope` during refresh, authenticate with the same OAuth client that issued the token, preserve rotated refresh tokens, and publish token state atomically. Streamrip now applies these behaviors. Its token store records `client_id`, rejects a known client mismatch, and writes by fsynced same-directory temporary file plus `os.replace`.
- Local metadata-only inspection (no tokens printed) showed tiddl's primary and TV fallback sessions valid into September 8, while Streamrip's separate primary and fallback token files expired September 6 and previously lacked stored client IDs. Streamrip therefore requires fresh independent authorization before a real TIDAL validation; silently sharing tiddl's rotating refresh token was rejected because simultaneous clients could invalidate one another.
- New regression coverage verifies rejected-session redaction and clean stop, continuation with an unexpired access token after preventive rejection, Basic client authentication without refresh `scope`, atomic token persistence with client identity, and the index's incremental/stale/duplicate/quality behavior. The uninterrupted suite passes `262 passed, 7 skipped`; Ruff passes. No real TIDAL request, login, media download, Git commit, or push was performed for this stage.

### Cross-service comparison repair for inconsistent catalog ISRCs

- A strict 16-bit artist run for TIDAL artist `3915689` incorrectly produced `NO MATCH` even though equivalent FLAC recordings exist. Two independent regressions were identified: generic Deezer text search was being used for raw ISRC values instead of Deezer's exact `track/isrc:` endpoint, and the legacy binary ISRC download database could bypass the newer quality-aware library index. Library-mode tracks now delegate cross-source existence decisions to the quality-aware index, and completion/index callbacks reject paths that were not actually created.
- The first affected recording provided a reproducible catalog inconsistency. TIDAL track `526967749` identifies `Memo Rex`, version `En Vivo`, duration 407 seconds, as ISRC `MXUM72503877`; Deezer track `4037457791` identifies the same artist, live title, and exact duration as `MXUM72503867`. Deezer's exact endpoint has no record for TIDAL's value. TIDAL also stores `En Vivo` in a separate version field that the common identity adapter previously discarded.
- The candidate adapter now preserves TIDAL edition/version text. Comparison remains exact-ISRC-first, but a conflicting provider ISRC may fall back only to the existing strict metadata proof: equal normalized title including edition, equal normalized artist, and duration within three seconds. This exception is local to cross-catalog comparison; the general ISRC matcher remains strict. Exact Deezer lookup failures fall through to ordinary metadata search instead of disabling the service.
- A live, read-only strict comparison of TIDAL track `526967749` was first limited to TIDAL and Deezer to isolate the broken Deezer path, then repeated with TIDAL, Deezer, and Qobuz enabled. It finds TIDAL equivalent `473470790` and Deezer equivalent `4037457791`, both FLAC/lossless/16-bit/44.1 kHz. Qobuz returned no verified equivalent for this recording; it did not error and remains part of the comparison. TIDAL wins only because equal delivered quality is resolved by the configured source priority. No media was downloaded.
- Regression coverage includes exact Deezer ISRC lookup, catalog-ISRC conflict acceptance only under strong metadata, rejection of a wrong edition/duration, quality-index precedence over the legacy ISRC flag, and refusal to index a nonexistent completion path. Focused comparison tests pass `39 passed`; the uninterrupted full suite passes `270 passed, 7 skipped`. Ruff passes after removal of an obsolete import. This stage is prepared for the next `main` publication; no credential, media file, tag, or release was changed.

### Configurable TIDAL 429 circuit breaker

- Continued successful downloads through `tiddl-elvigilante` confirmed that Streamrip's safety-stop message did not establish an account-wide block. Streamrip's local guard accumulated every HTTP 429 across the entire run and never reset after successful requests, so intermittent throttling could eventually terminate a healthy long-running artist job.
- `comparison.tidal_429_strike_limit` is now a permanent configuration field. Its default value `0` disables the run-wide circuit breaker while retaining request pacing, `Retry-After`, adaptive delay, and the bounded retry count of each individual request. Positive values enable a consecutive-429 threshold; any successful TIDAL response resets its strikes and tripped state.
- Regression coverage verifies disabled mode, consecutive strike behavior, reset after success, and rejection of negative configuration. Focused validation passed `39 passed`; the uninterrupted suite passed `272 passed, 7 skipped`; Ruff passed. No live media download or credential modification was performed for this correction.

### Strict Deezer comparison and regional fallback efficiency

- Artist-library output exposed repeated Deezer `album/.../tracks no data`, quality-0 fallback, and geolocation warnings while the final strict 16-bit selector still chose or skipped valid FLAC. These messages came from candidate inspection: it enriched every candidate with a complete album and attempted a lossy URL before the cross-service ceiling rejected it.
- Deezer candidate inspection now fetches only track metadata and resolved stream properties; complete album metadata remains deferred to the selected download path. `--no-fallback-to-lossy` is propagated into service inspection, so a candidate cannot request MP3 merely as an intermediate fallback.
- Deezer regional `FALLBACK` IDs are tried at the same requested quality before a strict candidate is rejected. Live read-only validation on Deezer track `86429847` now resolves its regional substitute as FLAC/lossless/16-bit/44.1 kHz without album-enrichment errors, quality-0 fallback, or geolocation warnings. No media was downloaded.
- Focused validation passed `24 passed, 1 skipped`; the uninterrupted suite passed `273 passed, 7 skipped`; Ruff passed. The change affects `streamrip/client/client.py`, `streamrip/client/deezer.py`, `streamrip/comparison.py`, `streamrip/rip/cli.py`, and comparison tests.

### Ordered lossless bit-depth selection

- `rip compare` and `rip library` now accept `--bit-depth-order`, with `16,24` implementing the requested policy: compare all available services at FLAC 16-bit first and open a 24-bit round only when no verified 16-bit candidate exists. Only listed lossless bit depths qualify; `--no-fallback-to-lossy` continues to exclude AAC, MP3, EAC3, and other lossy delivery.
- The implementation is round-based rather than a cosmetic ranking change. Service CD tiers run concurrently; a successful preferred-depth round stops later rounds, reducing API and stream-inspection work for mass artist/playlist jobs. When the CD round has no match, capable service clients are queried at their configured Hi-Res tier and the 24-bit result becomes eligible.
- `comparison.bit_depth_order` provides the permanent equivalent in `config.toml`, for example `[16, 24]`. An explicit `--max-bit-depth` overrides a configured order for one run; passing both quality options in the same command is rejected as ambiguous. Job signatures include the ordered policy so resume state cannot cross quality strategies.
- A live read-only comparison using Deezer reference track `86429847`, all three services, `--bit-depth-order 16,24`, and lossy fallback disabled selected Deezer FLAC 16-bit/44.1 kHz over an equal Qobuz candidate and did not enter the 24-bit round. TIDAL login independently reported its previously documented rejected saved OAuth session; the other services continued normally. No media was downloaded.
- Regression coverage proves 16-over-24 ordering, 24 fallback after a 16 miss, exclusion of unlisted depths, service-tier mapping, early termination before Hi-Res requests, CLI registration, and configuration persistence. The uninterrupted suite passes `278 passed, 7 skipped`; Ruff passes.
