...
- Residual gap versus tiddl: retained stages are reported in the error but are not yet indexed in a durable recovery registry/CLI, and Streamrip has no trusted destination-volume identity marker. Those controls should be ported before claiming full NAS/USB destination-safety parity.
